...
def parse_remote_id_element(element_data: bytes) -> dict | None:
...
def parse_basic_id(data: bytes) -> dict:
    ...
    try:
        ...
        id_types = {0: "None", 1: "Serial Number", 2: "CAA Registration", 3: "UTM Assigned", 4: "Specific Session"}
...
def parse_self_id(data: bytes) -> dict:
    ...
    try:
        ...
        return {"description_type": desc_types.get(desc_type, f"Unknown({desc_type})"), "description": description}
    except Exception:
...
def parse_operator_id(data: bytes) -> dict:
    ...
    try:
        ...
        return {"operator_id_type": op_id_type, "operator_id": operator_id}
    except Exception:
...
def main(argv: list[str] | None = None) -> int:
    """Main function."""
    parser = argparse.ArgumentParser(description="Capture drone Remote ID information over WiFi")
    parser.add_argument("interface", help="WiFi interface name (e.g., wlan0, en0, wlp2s0)")
